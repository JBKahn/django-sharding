...
def model_config(shard_group=None, database=None):
    ...
    def configure(cls):
        if database and shard_group:
            raise ShardedModelIntializationException('A model cannot be both sharded and stored on a particular database.')

        if not database and not shard_group:
            raise ShardedModelIntializationException('The model should be either sharded or stored on a database in the `model_config` decorator is used.')
        ...
        if shard_group:
            ...
            if not sharded_fields:
                raise ShardedModelIntializationException('All sharded models require a ShardedIDFieldMixin.')

            if not filter(lambda field: field == cls._meta.pk, sharded_fields):
                raise ShardedModelIntializationException('All sharded models require the ShardedAutoIDField to be the primary key. Set primary_key=True on the field.')

            if not callable(getattr(cls, 'get_shard', None)):
                raise ShardedModelIntializationException('You must define a get_shard method on the sharded model.')
