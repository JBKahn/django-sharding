...
def model_config(shard_group=None, database=None, is_linting=False):
    """
    ...
    def configure(cls):
        ...
        if database:
            if database not in settings.DATABASES or settings.DATABASES[database].get('PRIMARY') and is_linting is False:
        ...
        if postgres_shard_id_fields and is_linting is False:
